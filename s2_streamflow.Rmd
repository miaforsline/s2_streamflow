...
#create filepath to call the data from Box 
...
# Combine water year and precipitation data 
...
p <- ggplot() + 
  geom_line(data = streamflow_precip, 
            aes(x = datetime, 
                y = south_pcp,
                colour = I("grey"))) + 
  geom_point(data = streamflow_precip[!is.na(streamflow_precip$stream_height_ft),],
             aes(x = datetime, 
                 y = stream_height_ft, 
                 color = "red"), 
             size = 0.1) + 
...
```{r}
all <- full_join(x = streamflow_precip, y = bogwell_clean, by = "date")

```


# Compare stream height and precipitation using `patchwork`

```{r}
a <- ggplot(data = all_streamflow) + 
  geom_line(aes(x = datetime, y = stream_height_ft)) + 
  theme_classic() + 
  labs(x = "Time", 
       y = "Stream Height (Ft)",
