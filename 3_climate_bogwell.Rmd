...
Box\External-MEF_DATA\Hydro\WaterTableElevation\Bogwells\L1\ L1_Peatland_well_daily_history_2021forMia.csv
```{r bogwell_s2}
#set file path to read in data from Box 
...
#save clean data CSV to use in future RMD files
write.csv(x = bogwell_s2_clean, 
          file = file.path(here("intermediate_data", "bogwell_s2_clean.csv")),
          row.names = FALSE)
```

```{r bogwell_s2_plot}
#plot
p_bogwell_s2 <- ggplot(data = bogwell_s2_clean) + 
...
S2_bogwell_S2BW_Daily.dat (MaxWTElev)

Handy function for loading Campbell DataLogger .dat files:
```{r dat_function}
...
Constants I used for converting between feet and meters:
```{r constants}
